...
- **Important files**: `pytest.ini`, `.github/workflows/ci.yml` (unchanged; it now automatically consumes the root config). The root pytest config explicitly includes both Python project roots plus Runtime `src`.
