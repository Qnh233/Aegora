...
1. Workflow Capability now includes the first governance-editing slice: administrators can register governed workflow capabilities backed by an existing MCP connection, Runtime reuses the MCP execution adapter while preserving `source=workflow`, and the console can explicitly edit Input Schema, scope allow-list schema and scope descriptions. Next, add workflow version/lifecycle management.
