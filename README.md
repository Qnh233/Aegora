...
The current codebase already contains foundations for this design: immutable releases, runtime policy resolution, governance/audit boundaries, Skills, the existing reflection/Skill-draft path, a central LiteLLM gateway integration and Langfuse trace correlation. Runtime chat persistence now carries Agent/Release lineage plus an immutable Release-scoped learning-policy snapshot into reflection evidence, and reflection clustering never mixes evidence across Agents. Evidence collection can be disabled per Agent, while Skill proposal generation is explicit opt-in and otherwise falls back to human learning review. `source=agent` Skills are blocked from promotion unless they carry a passed evaluation record with dataset/metrics/timestamp plus an explicit human reviewer; evaluation evidence is bound to the exact candidate content hash, so editing a Skill after evaluation invalidates the gate. Promotion evidence must also include explicit criteria, a passed regression comparison against a prior accepted evaluation baseline, and a passed Canary record bound to the same content hash with a declared rollout mode, non-zero sample size, metrics and observation time. The gate is enforced both for local promotion and Strapi-to-PostgreSQL sync. Producing Canary evidence from real bounded traffic and the remaining full flywheel automation remain roadmap work.
